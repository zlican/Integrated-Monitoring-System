<template>
  <CardFrame :updatedAt="updatedAt">
    <template #title>CEX监控区(长线)</template>
...
import type { CexMessage, CexMessageL } from '@/types';
...
const handleClick = (text: string) => {
  ...
  if (symbolMatch) {
    ...
    const url = `https://www.tradingview.com/chart/XwoDRBjf/?symbol=BINANCE%3A${symbol}USDT`;
    window.open(url, '_blank');
    return;
  }
