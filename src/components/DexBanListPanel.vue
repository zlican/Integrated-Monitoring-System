<template>
    <CardFrame :updatedAt="updatedAt">
      <template #title>DEX封禁区管理</template>
  ...
  const fetchSymbols = async () => {
    try {
      const res = await fetch(`http://${ip}:9001/dex/ban/list`);
  ...
  try {
    await fetch(`http://${ip}:9001/dex/ban/add`, {
      method: "POST",
  ...
  const handleRemove = async (symbol: string) => {
    ...
    try {
      await fetch(`http://${ip}:9001/dex/ban/remove/${symbol}`, { method: 'DELETE' });
      fetchSymbols();
    } catch (err) {
